/**
 * @file mef.c
 * @brief MEF 48/49 Carrier Ethernet Performance Testing
...
int mef_config_test(rfc2544_ctx_t *ctx, const mef_config_t *config, mef_config_result_t *result)
{
    ...
    for (uint32_t s = 0; s < num_steps && !ctx->cancel_requested; s++) {
        ...
        int ret = run_trial_custom(ctx, frame_size, rate_pct, config->config_test_duration_sec,
                                   MEF_DEFAULT_WARMUP_SEC, MEF_SIGNATURE, s + 1, &trial);
    ...
    rfc2544_log(LOG_INFO, "Configuration Test: %s", result->overall_passed ? "PASS" : "FAIL");
...
int mef_perf_test(rfc2544_ctx_t *ctx, const mef_config_t *config, mef_perf_result_t *result)
{
    ...
    rfc2544_log(LOG_INFO, "Service: %s, Duration: %u min (%u sec)", config->service_id,
                config->perf_test_duration_min, duration_sec);
    ...
    trial_result_t trial;
    int            ret = run_trial_custom(ctx, 1518, rate_pct, duration_sec, MEF_DEFAULT_WARMUP_SEC,
    ...
    result->overall_passed =
        result->fd_passed && result->fdv_passed && result->flr_passed && result->avail_passed;

    rfc2544_log(LOG_INFO, "Performance Results:");
    rfc2544_log(LOG_INFO, "  Throughput: %u kbps", result->throughput_kbps);
    rfc2544_log(LOG_INFO, "  FD: avg=%.1f, min=%.1f, max=%.1f us", result->fd_avg_us,
                           ...
                           const mef_perf_result_t   *perf_result)
{
    ...
    if (perf_result) {
        printf("{\"service_id\":\"%s\","
               ...
               perf_result->fd_passed ? "true" : "false",
               perf_result->fdv_passed ? "true" : "false",
...
void mef_print_sla_report(const mef_sla_report_t *report)
{
    ...
    printf("Frame Delay:     %s (threshold: %.1f us, measured: %.1f us, margin: %.1f us)\n",
           report->fd_compliant ? "COMPLIANT" : "NON-COMPLIANT", report->fd_threshold_us,
           report->fd_measured_us, report->fd_margin_us);
    printf("Delay Variation: %s (threshold: %.1f us, measured: %.1f us, margin: %.1f us)\n",
           report->fdv_compliant ? "COMPLIANT" : "NON-COMPLIANT", report->fdv_threshold_us,
           report->fdv_measured_us, report->fdv_margin_us);
    printf("Frame Loss:      %s (threshold: %.4f%%, measured: %.4f%%, margin: %.4f%%)\n",
           report->flr_compliant ? "COMPLIANT" : "NON-COMPLIANT", report->flr_threshold_pct,
           report->flr_measured_pct, report->flr_margin_pct);
    printf("Availability:    %s (threshold: %.4f%%, measured: %.4f%%, margin: %.4f%%)\n",
           report->avail_compliant ? "COMPLIANT" : "NON-COMPLIANT", report->avail_threshold_pct,
           report->avail_measured_pct, report->avail_margin_pct);
