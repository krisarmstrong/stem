...
func TestNewAPIError(t *testing.T) {
	...
	apiErr := NewAPIError(http.StatusNotFound, ErrCodeNotFound, "Resource not found")
...
func TestErrorHelpers(t *testing.T) {
	...
	tests := []struct {
		name         string
		createErr    func() *APIError
		expectedCode ErrorCode
		expectedHTTP int
	}{
		{
			name:         "InvalidRequestError",
			createErr:    func() *APIError { return InvalidRequestError("bad input") },
			expectedCode: ErrCodeInvalidRequest,
			expectedHTTP: http.StatusBadRequest,
		},
		{
			name:         "NotFoundError",
			createErr:    func() *APIError { return NotFoundError("item not found") },
			expectedCode: ErrCodeNotFound,
			expectedHTTP: http.StatusNotFound,
		},
		{
			name:         "ConflictError",
			createErr:    func() *APIError { return ConflictError("already exists") },
...
func TestPredefinedErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "ErrAPIAuthFailed",
			apiErr:       ErrAPIAuthFailed,
			expectedCode: ErrCodeAuthFailed,
			expectedHTTP: http.StatusUnauthorized,
		},
		{
			name:         "ErrAPIAuthExpired",
			apiErr:       ErrAPIAuthExpired,
			expectedCode: ErrCodeAuthExpired,
			expectedHTTP: http.StatusUnauthorized,
		},
		{
			name:         "ErrAPIPermissionDenied",
			apiErr:       ErrAPIPermissionDenied,
			expectedCode: ErrCodePermissionDenied,
			expectedHTTP: http.StatusForbidden,
		},
		{
			name:         "ErrAPIMethodNotAllowed",
			apiErr:       ErrAPIMethodNotAllowed,
			expectedCode: ErrCodeMethodNotAllowed,
			expectedHTTP: http.StatusMethodNotAllowed,
		},
		{
			name:         "ErrAPIInvalidJSON",
			apiErr:       ErrAPIInvalidJSON,
			expectedCode: ErrCodeInvalidRequest,
			expectedHTTP: http.StatusBadRequest,
		},
		{
			name:         "ErrAPIRequestTooLarge",
			apiErr:       ErrAPIRequestTooLarge,
			expectedCode: ErrCodeInvalidRequest,
			expectedHTTP: http.StatusRequestEntityTooLarge,
		},
		{
			name:         "ErrAPINotFound",
			apiErr:       ErrAPINotFound,
			expectedCode: ErrCodeNotFound,
			expectedHTTP: http.StatusNotFound,
		},
		{
			name:         "ErrAPIConflict",
			apiErr:       ErrAPIConflict,
			expectedCode: ErrCodeConflict,
			expectedHTTP: http.StatusConflict,
		},
		{
			name:         "ErrAPIRateLimited",
			apiErr:       ErrAPIRateLimited,
			expectedCode: ErrCodeRateLimited,
			expectedHTTP: http.StatusTooManyRequests,
		},
		{
			name:         "ErrAPIInternalError",
			apiErr:       ErrAPIInternalError,
			expectedCode: ErrCodeInternalError,
			expectedHTTP: http.StatusInternalServerError,
		},
		{
			name:         "ErrAPIServiceUnavailable",
			apiErr:       ErrAPIServiceUnavailable,
			expectedCode: ErrCodeServiceUnavailable,
			expectedHTTP: http.StatusServiceUnavailable,
		},
	}
