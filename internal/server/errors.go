...
// APIErrorResponse represents a standardized error response for the API.
...
// APIError represents an error with associated HTTP status and error code.
...
// NewAPIErrorWithCause creates a new APIError that wraps an underlying error.
// The internal error is logged but not exposed to clients.
func NewAPIErrorWithCause(status int, code ErrorCode, message string, cause error) *APIError {
	return &APIError{
		HTTPStatus:  status,
		Code:        code,
		Message:     message,
...
// ToResponse converts the APIError to an APIErrorResponse suitable for JSON encoding.
...
var (
	// ErrAPIAuthFailed is returned when authentication fails.
	ErrAPIAuthFailed = &APIError{
	...
	// ErrAPIAuthExpired is returned when the authentication token has expired.
	ErrAPIAuthExpired = &APIError{
		HTTPStatus:  http.StatusUnauthorized,
	...
	// ErrAPIAuthRevoked is returned when the authentication token has been revoked.
	ErrAPIAuthRevoked = &APIError{
		HTTPStatus:  http.StatusUnauthorized,
		Code:        ErrCodeAuthFailed,
	...
	// ErrAPIPermissionDenied is returned when the user lacks required permissions.
	ErrAPIPermissionDenied = &APIError{
	...
	// ErrAPIMethodNotAllowed is returned when the HTTP method is not allowed.
	ErrAPIMethodNotAllowed = &APIError{
	...
	// ErrAPIInvalidJSON is returned when the request body contains invalid JSON.
	ErrAPIInvalidJSON = &APIError{
	...
	// ErrAPIRequestTooLarge is returned when the request body is too large.
	ErrAPIRequestTooLarge = &APIError{
	...
	// ErrAPINotFound is returned when a resource is not found.
	...
	// ErrAPIConflict is returned when there is a resource conflict.
	ErrAPIConflict = &APIError{
	...
	// ErrAPIRateLimited is returned when the rate limit is exceeded.
	ErrAPIRateLimited = &APIError{
	...
	// ErrAPIInternalError is returned when an internal server error occurs.
	ErrAPIInternalError = &APIError{
	...
	// ErrAPIServiceUnavailable is returned when the service is temporarily unavailable.
...
func InvalidRequestError(message string) *APIError {
	return NewAPIError(http.StatusBadRequest, ErrCodeInvalidRequest, message)
}
...
// MapAuthError converts an auth package error to an appropriate APIError.
...
// MapTestError converts a test execution error to an appropriate APIError.
// This centralizes the mapping of test-related errors to HTTP responses.
func MapTestError(err error) *APIError {
	switch {
...
// WriteAPIError writes an APIError to the response as JSON.
...
// It attempts to convert the error to an APIError; if not possible,
...
func WriteInvalidRequest(w http.ResponseWriter, message string) {
	WriteAPIError(w, InvalidRequestError(message))
}

// WriteConflict writes a 409 Conflict response with a custom message.
func WriteConflict(w http.ResponseWriter, message string) {
	WriteAPIError(w, ConflictError(message))
}
